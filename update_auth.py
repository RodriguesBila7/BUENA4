...
# 1. Update State
state_target = '''  const [username, setUsername]       = useState('');
...
state_repl = '''  const [username, setUsername]       = useState('');
  const [password, setPassword]       = useState('');
...
# 2. Update Handlers
handlers_target = '''  /* ── Autenticação simulada ── */
...
handlers_repl = '''  /* ── Autenticação e Registo ── */
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!username.trim() || !password.trim()) {
      setError('Por favor, preencha todos os campos.');
...
# 3. Update the Card rendering
card_target = '''      {/* ══════════════ CARD DE LOGIN ══════════════ */}
      <div style={s.card}>
...
card_repl = '''      {/* ══════════════ CARD DE LOGIN / REGISTO / RECUPERAR ══════════════ */}
      <div style={s.card}>
...
# 4. Add linkRow and linkBtn to styles
style_target = '''  btnSecondary: {
    width: '100%',
    padding: '13px',
